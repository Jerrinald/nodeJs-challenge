module.exports = function Controller(Service, options = {}) {
    return {
      ...
      // Nouvelle méthode pour récupérer les commandes par numéro de commande
      getByNumeroCommande: async (req, res, next) => {
        const { numeroCommande } = req.query;
        try {
          const results = await Service.findByNumeroCommande(numeroCommande);
          res.json(results);
        } catch (err) {
          next(err);
        }
